...
class CantileverEnv_v0_1(gymnasium.Env):
    ...
    def __init__(self, config: dict) -> None:
        ...
        self.eps_length = config.get("eps_length",200)
        # Set up action and observation spaces
        self.action_space = spaces.MultiDiscrete([self.num_sensors, len(self.ACTIONS)], seed=self.seed)
        num_nodes = 1462
        self.observation_space = spaces.Box(low=0, high=1, shape=(num_nodes,), dtype=np.int8, seed=self.seed)

        # Initialize PyAnsys environment and state variables
        self.pyansys_env = config.get('pyansys_env')
        #Generate list of mode shaoe matric for damage cases
        self.phi = self.pyansys_env.phi
        self.correlation_covariance_matrix = self.pyansys_env.correlation_covariance_matrix
                    
        #Direction Adjsutments
        self.adjustments = [
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, bool, dict]:
        ...
        Args:
            action (np.ndarray): Action array in the form [sensor_id, direction, step].

        Returns:
            Tuple containing the new observation, reward, termination status, and info dict.
        """
        try:
            self.step_left -= 1
            ...
            return self.current_state_binary, reward, done, False, info
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        try:
            ...
            self.step_left = self.eps_length
            ...
            return self.current_state_binary , info
    ...
    def _initialize_state(self):
        """
        Initializes the starting state for the sensors.
        """
